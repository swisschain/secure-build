...
cat <<EOF > sbs-config.json
{
    ...
    "REPO_ID": "sbs",
    "DOCKER_REPO": "$CR_NAME-$release/$CONTAINER_NAME",
    "DOCKER_USER": "iamapikey",
    "DOCKER_PASSWORD": "$IAMAPI_KEY",
    "DOCKER_RO_USER": "iamapikey",
    "DOCKER_RO_PASSWORD": "$IAMAPI_KEY",
    "DOCKER_CONTENT_TRUST_BASE": "False",
    "DOCKER_CONTENT_TRUST_BASE_SERVER": "",
    "DOCKER_BASE_SERVER": "de.icr.io",
    "DOCKER_PUSH_SERVER": "de.icr.io",
    "DOCKER_CONTENT_TRUST_PUSH_SERVER": "de.icr.io",
    "ENV_WHITELIST": [
        "ZHSM",
        "APIKEY",
        "IAM_ENDPOINT",
...
cat <<EOF > temp
ibmcloud hpvs instance-create $BUILD_SERVER free fra05 --rd-path "secure_build.asc" -i 1.3.0.11 $env_certs
EOF
echo cat temp
cat temp
sh temp
