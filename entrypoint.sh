...
rm ~/.ssh/temp
git clone --single-branch --branch master "https://x-access-token:$GIT_USER_API_TOKEN@github.com/swisschain/custody-certificates/ /tmp/git
cd custody-certificates/$SERVICE_NAME
